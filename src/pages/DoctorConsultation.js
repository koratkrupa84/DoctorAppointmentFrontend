...
const DoctorConsultation = () => {
  ...
  const fetchConsultations = async () => {
    try {
      const res = await fetch(API.DOCTOR_CONSULTATIONS, {
  ...
  const fetchMessages = async (consultationId) => {
    try {
      const res = await fetch(`${API.DOCTOR_CONSULTATION_MESSAGES}/${consultationId}/messages`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (res.ok) {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      const res = await fetch(
        `${API.DOCTOR_SEND_CONSULTATION_MESSAGE}/${selectedConsultation.id}/message`,
        {
          method: "POST",
          headers: {
                ...
                {messages.length === 0 ? (
                  ...
                  messages.map((msg) => (
                    <div
                      key={msg.id}
                      className={`message ${
                        msg.sender_role === "Doctor" ? "sent" : "received"
                      }`}
                    >
