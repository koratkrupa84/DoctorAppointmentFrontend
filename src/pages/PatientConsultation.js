...
const PatientConsultation = () => {
  ...
  const fetchConsultations = async () => {
    try {
      const res = await fetch(API.PATIENT_CONSULTATIONS, {
  ...
  const fetchDoctors = async () => {
    try {
      const res = await fetch(API.CONSULTATION_DOCTORS, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (res.ok) {
              ...
              consultations.map((consultation) => (
                ...
                >
                  <div className="consultation-info">
                    <strong>Dr. {consultation.doctor_name}</strong>
          ...
          {selectedConsultation ? (
            <>
              <div className="chat-header">
                <div>
                  <h3>Dr. {selectedConsultation.doctor_name}</h3>
                ...
                {messages.length === 0 ? (
                  ...
                  messages.map((msg) => (
                    <div
                      key={msg.id}
                      className={`message ${
                        msg.sender_role === "Patient" ? "sent" : "received"
                      }`}
                    >
